...
namespace fitnesse.mtee.operators {
    public class DefaultRuntime<T>: RuntimeOperator<T> {
        public bool TryCreate(Processor<T> processor, string memberName, Tree<T> parameters, ref object result) {
            ...
            if (parameters.Branches.Count == 0) {
                ...
                result = member.Invoke(runtimeType.Type, GetParameterList(processor, parameters, member)).Value;
            }
            return true;
        }

        public bool TryInvoke(Processor<T> processor, Type type, object instance, string memberName, Tree<T> parameters, ref TypedValue result) {
            RuntimeMember member = new RuntimeType(type).GetInstance(memberName, parameters.Branches.Count);
            result = member.Invoke(instance, GetParameterList(processor, parameters, member));
            return true;
        }

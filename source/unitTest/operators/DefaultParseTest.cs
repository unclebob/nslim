...
namespace fitnesse.unitTest.operators {
    [TestFixture] public class DefaultParseTest {
        ...
        [Test] public void StringIsParsed() {
            object result = null;
            parse.TryParse(processor, typeof (string), null, new TreeLeaf<string>("stuff"), ref result);
            Assert.AreEqual("stuff", result);
        }

        [Test] public void DateIsParsed() {
            object result = null;
            parse.TryParse(processor, typeof(DateTime),null, new TreeLeaf<string>("03 Jan 2008"), ref result);
            Assert.AreEqual(new DateTime(2008, 1, 3), result);
        }

        [Test] public void ClassIsParsed() {
            object result = null;
            parse.TryParse(processor, typeof(SampleClass), null, new TreeLeaf<string>("stuff"), ref result);
